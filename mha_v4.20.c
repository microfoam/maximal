...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.20";				/* current version number */
	...
	if (options[0][13]) {	/* opt_D: SHOW DTHR VALUES */
		...
		for (k = 1; k <= WIDTH; k++) {
			if (seqtype==1 && k>PISO) {
				n = (int) round(0.32*log2((float) k));
				i = score_DTHR(k);
				j = (int) 100*((k-n)*match + n*transition)/(k*match);
			}
			else {
				n = 0;
				i = j = 100;
					...
					if (tela[n].X != n) {
						for (l=n-1; l>=tela[n].X; l--) {
							if ((q=tela[l].k) && (l + (p=span_rk(l)) - q) > m) { 
								...
								else if (l < m && tela[l].cyc_l==1 && tela[n].c != tela[n+k].c) {	
                            ...
                            if (tela[i].r) {
                                if (!badslip_type) {
                                    ...
                                    if (i>m && i - tela[i].k < m && i%tela[i].k != m%tela[i].k) {
										...
										assign_tela(i,tela[i-1].y,tela[i-1].x+1, ONE);	/* MODES O-F-F, ONE--FIVE: ONE=FLATLINE AT N, TWO=ASSIGN  */
                                    }    
	        	                    /* THIS IS A BAD SLIP BECAUSE OF NON-COINCIDENT TRANSITIONS  */
										...
										/* THIS IS BAD SLIP WITH MISMATCH O-F-FSET FWRD FROM tela[i].r */
											...
											if (p< tela[i].k) {
												...
												options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
												if (dev_print(MAIN,__LINE__)) {
													printf("badslip type %d at n=%d for k=%d with TR at p=%d.", badslip_type, n, k, p);
												}
					...
					for (i = 0; i < r; i++) {
						...
						if (badslipspan && i==0) {			/* FLIP TO LOWERCASE IN FIRST UNIT */	
							if (islower(align2D[(p=row-1)][(q=a2D_n - k + overslip - 1)])) 		/* IF OVERLAPPING OLD BADSLIP */
								assign_tela(m-1, p, q, TWO);		/* MODES O-F-F, ONE--FIVE: ONE=FLATLINE AT N, TWO=ASSIGN  */
...
*/					}
					...
					if (badslip_type >= 100) {
						tela[bad_1Dn].r = 0;
						assign_tela(n,tela[n-1].y,tela[n-1].x+1, ONE);	/* MODES O-F-F, ONE--FIVE: ONE=FLATLINE AT N, TWO=ASSIGN  */
					}
