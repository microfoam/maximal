...
{
	int match      =  MATCH;				/* MOVE SCORE RELATED STUFF TO HEADER FILE */
	int transition =  match/2;			
	int mismatch   = -1;
	char version[] = "4.10";				/* current version number */
	...
	if (options[0][13]) {	/* opt_D: SHOW DTHR VALUES */
		...
		for (k = 1; k <= WIDTH; k++)
			printf("\n %3d\t %d\t %12d%%.", k, 1 + (int) round(fractransit*k), score_DTHR(k));
	...
	mark_all();			/* WILL MARK ALL TRs WITHOUT CINCHING AND RECORD IN tela[].all_k, all_r, all_S, all_L/R */
	for (n = 1; n<=lenseq; ) {

		/* FOR COLUMN n LOOP 1/3 */
		if (!options[0][55]) {		/* SKIP TO NEXT MARKED TR */	
			while (!(tela[n].all_k) && n!=lenseq) {
		...
		if (tela[n].all_L && tela[(tela[n].all_L)].cyc_o == 'x' && !tela[n+1].all_L && tela[n].all_S == tela[n+1].all_S) 
			assign_tela(n++, row, a2D_n++, 0,0,0);
		...
		for (m = 0; m < n; m++) {
			...
			if (pathbox[m][n] > mismatch && n+k <= lenseq) {
				...
				if (options[1][57] > 3 && Dtr)	/* VERBOSITY */
					printf("\nDEV-0762: At m=%d and n=%d, k=%d, all_r=%d, imperfect_TR=%d.", m, n, k, tela[n].all_r, imperfect_TR);
					...
					if (tela[n].X != n) {
						for (l=n-1; l>=tela[n].X; l--) {
							if ((q=tela[l].k) && (l + (p=span_rk(l)) - q) > m) { 
								...
								else if (imperfect_TR && (tela[n].all_S < tela[l].all_S || 	/* SKIP THIS SLIP IF OVERLAPPING IN CONFLICT PREV. TR W/ > SCORE OR */
										(tela[n].all_S == tela[l].all_S && tela[n].all_k > tela[l].all_k)) ) {	
									badslip_type = 3;					/* FROM SEQUENCE IN TYPES: 1- (3) -5-10-30-50-100-300-500 */
									if (tela[n].all_S == tela[l].all_S && tela[n].all_k > tela[l].all_k)
										badslip_type = 3000;
									options[1][50] += badslip_type;
									options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
									if (options[1][57] > 3) /* opt_v DEV-LEVEL VERBOSITY */
								...
								else if (!imperfect_TR && l >= m && l+span_rk(l) < n)
									;
				...
				if (Dtr && tela[n].all_S != tela[n].all_r *k*match && (o=push_tela(n,m, 3))) {		/* PUSH_TELA() WITH ONLY AXIOMATIC TEST 1 (IS GOAL/PLAN) */
					if (options[1][57] > 3) { /* opt_v DEV-LEVEL VERBOSITY */
						printf("\nDEV-0921: push_tela violations=%d (+1 CONT, +2 EQUIV). Skipping imperfect k=%d-mer at n=%d.", o, k, n+1);
						print_tela(prtela_A, prtela_B);
					}
					Dtr = imperfect_TR = 0;
					pull_tela(n);
				}
				else if (Dtr && (o=push_tela(n,m,3))) {		/* PUSH_TELA() WITH BOTH AXIOMATIC TESTS 1 + 2 */
					if (options[1][57] > 3) { /* opt_v DEV-LEVEL VERBOSITY */
						printf("\nDEV-0921: push_tela violations=%d (+1 CONT, +2 EQUIV). Skipping k=%d-mer at n=%d.", o, k, n+1);
						print_tela(prtela_A, prtela_B);
					}
					Dtr = imperfect_TR = 0;
					pull_tela(n);
				...
				/* IF SUMMING PATHBOX DIAGONAL 8/:  1st MEASUREMENT OF TANDEM REPEAT (TR) */ 
				else if (Dtr==Did || imperfect_TR) {	
					tela[n].Dtr = Dtr;		/* SAVE Dtr SCORE */

					if (k > options[1][46])	/* FOR RECORDING IN MAX K SLOT */
						options[1][46] = k;
							...
							if (!badslip_type) {
								...
								if (sumspan > 0 && tela[n].X != n) {	/* SUMSPAN IS LENGTH OF WINDOWS FOR WHICH SUMS OF PRODUCTS ARE RECORDED */
									...
									if (l != z && tela[(tela[l].cyc_Lf)].cyc_o == 'x') {
										...
										options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
										if (options[1][57] > 3) { /* opt_v DEV-LEVEL VERBOSITY */
											printf("\nDEV-1097: badslip type %d at n=%d for k=%d with TR at l=%d.", badslip_type, n, k, l);
											printf("\nDEV-1098: Before--->");
											print_tela(prtela_A, prtela_B);
										}
										...
										o = push_tela(l,l-k, 3);
										if (options[1][57] > 3)  /* opt_v DEV-LEVEL VERBOSITY */
											printf("\nDEV-1114: push_tela violations = %d.", o);
										...
										if (tela[i].cyc_o == 'x') {
											if (cyclelize_tela(i, j-i, n)) {
												...
												options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
												if (options[1][57] > 3) /* opt_v DEV-LEVEL VERBOSITY */
													printf("\nDEV-1127: badslip type %d at n=%d for k=%d with TR at l=%d, delta=%d.", badslip_type, n, k, l, j-i);
												...
												if (options[1][57] > 3) /* opt_v DEV-LEVEL VERBOSITY */
													printf("\nDEV-1189: badslip type 10000 at n=%d; failed cyclelize_tela().", n);
											}
										}
									}
								}
									...
									if (cycto != z) {
										if (cyclelize_tela(z, cycto-z, n)) {	/* REMINDER: cyclelize_tela(int cpos, int delta, int npos) */
											...
											options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
											if (options[1][57] > 3) /* opt_v DEV-LEVEL VERBOSITY */
												printf("\nDEV-1169: badslip type %d at n=%d for k=%d with TR at cycto=%d, z=%d.", badslip_type, n, k, cycto, z);
					...
					if (options[1][57] > 2) {	/* opt_v ACTIVE DEV-LEVEL VERBOSITY */
						...
						printf("\nDEV-1431: print_tela for k=%d and r=%d at n=%d, overslip=%d.", k, r, n, overslip);
						print_tela(prtela_A, prtela_B);
					}
...
int score_DTHR(int kmer)
{
	...
	int transition = MATCH/2;		/* MOVE ME TO HEADER FILE */
	...
	if (axioms==1 || axioms==3) {
		...
		for (i=0; i<k; i++) {
			...
			if      (coord_yb == coord_ya && coord_xb == coord_xa + 1) 
				...
				violation = 1;
				printf("\nDEV-2037: push_tela() viol-1 at i=%d, k=%d, n2=%d, xa=%d xb=%d, ya=%d yb=%d.", i, k, n2, coord_xa, coord_xb, coord_ya, coord_yb); 
				break;
			}
		}
	}
	...
	if (axioms==2 || axioms==3) {
		...
		for (i=0; i<k; i++) {
			if      (tela[n1+i].c == tela[n2+i].c)
				...
				violation += 2;
				printf("\nDEV-2050: push_tela() viol-2 at %d and %d for k=%d.", n1+i, n2+i, k);
				break;	
			}
		}
	}
...
int max_k = (int) options[1][46];				/* MAX k-SIZE FROM CINCH-T */
int lenseq = options[1][1];
int *x_history = NULL;
...
*/
	...
	if (options[1][57]>3) {	/* opt_v VERBOSITY, DEV-LEVEL */
		printf("\nDEV-2571: Post-cinch-t max_k = %d.", max_k);
	}
	if (!max_k)
		max_k = WIDTH;
			...
			for (n = 0; align2D[m][n] != '\0'; n++) { 
				...
				if (keep_checking) {
					...
					for (l = 0; l < k; l++) {
						...
						/* FOR AESTHETICS? DON'T SCOOCH MONOS INTO TRANSITS */
						else if (k==1 && (consensus[n]=='R' || consensus[n]=='Y')) {
							keep_checking = 0;
							break;
						}
						else {
...
int consensus_ar[26][MAXROW] = {{0}};	 	/* COL n=0 FOR BIT FLAG */
	...
	for (n = n_start; n <= n_end; n++) {
		...
		if (checktransit) {		/* IF checktransit IS STILL POSITIVE THEN IT'S SUPPOSED TO NOT HAVE CHECKED OUT */
			if (1 || options[1][57]>2) {		/* opt_v VERBOSITY */
				printf("DEV-3392: checktransit=%d at n=%d.\n", checktransit, n);
...
void mark_all(void) 
{
	...
	for (n=lenseq; n>0; n--) {
		if (tela[n].all_k) {
			...
			m = n - k;
			j = n - 1;
			while (tela[j].all_k == k)		/* SKIP CYCLE COLUMNS OF SAME K-MER */
				j--;
			for (i=j; i>0; i--) {
