...
#include "microhom-skor.h"	/* maximal header 2 */
#include "microhom-tela.h"	/* maximal header 3 */
...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.12";				/* current version number */
				...
				if (nuctransit && Dtr && Dtr!=Did) { 
					if (k>PISO && 100*Dtr/Did > DTHR)	{	
						if (tela[m].c != tela[n].c && tela[n].c == tela[n+k].c)	{ 
							...
							tela[n].Dtr = Dtr;
							assign_transit(n,OFF); 	/* 0=OFF; 1=ALL_K/R; 2=CYC_K/R; 3=K/R */
						}
					}
					else 
						Dtr = 0;
