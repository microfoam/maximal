...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.11";				/* current version number */
		...
		for (m = 0; m < n; m++) {
			...
			if (pathbox[m][n] > mismatch && n+k <= lenseq) {
					...
					if (k>PISO && 100*Dtr/Did > DTHR)	{	
						if (tela[m].c != tela[n].c && tela[n].c == tela[n+k].c)	{ 
							...
							if (options[1][57] > 3) /* opt_v DEV-LEVEL VERBOSITY */
								printf("\nmain-DEV-0753: Skipping imperfect TR at n=%d for better match at n=%d.", n+1, n+2);
						}
						else
				...
				if (options[1][57] > 3 && Dtr)	/* VERBOSITY */
					printf("\nmain-DEV-0763: At m=%d and n=%d, k=%d, all_r=%d, imperfect_TR=%d.", m, n, k, tela[n].all_r, imperfect_TR);
...
*/									break;
				...
				if (nuctransit && Dtr && tela[n].all_S != tela[n].all_r *k*match) {
					imperfect_TR++;
					assign_transit(n,0); 	/* 0=OFF; 1=ALL_K/R; 2=CYC_K/R; 3=K/R */
				}
				/* IF SUMMING PATHBOX DIAGONAL 7/: COMMITTING TO CINCH AFTER AXIOMATIC TEST BY PUSH_TELA  ************************/
				if (Dtr && imperfect_TR && (o=push_tela(n,m,3))) {		/* PUSH_TELA() WITH ONLY AXIOMATIC TEST 1 (IS GOAL/PLAN) */
					if (options[1][57] > 3) { /* opt_v DEV-LEVEL VERBOSITY */
						printf("\nmain-DEV-0926: push_tela violations=%d (+1 CONT, +2 EQUIV). Skipping imperfect k=%d-mer at n=%d.", o, k, n+1);
					...
					if (options[1][57] > 3) { /* opt_v DEV-LEVEL VERBOSITY */
						printf("\nmain-DEV-0934: push_tela violations=%d (+1 CONT, +2 EQUIV). Skipping k=%d-mer at n=%d.", o, k, n+1);
						print_tela(prtela_A, prtela_B);
					}
					Dtr = imperfect_TR = 0;
					pull_tela(n);
				...
				/* IF SUMMING PATHBOX DIAGONAL 8/:  1st MEASUREMENT OF TANDEM REPEAT (TR) */ 
					...
					while (TRcheck) {
						...
						if (r<reps) {
							...
							if (imperfect_TR)
								assign_transit(n,3); 	/* 0=OFF; 1=ALL_K/R; 2=CYC_K/R; 3=K/R */
							if (!badslip_type) {
					...
					if (options[1][57] > 2) {	/* opt_v ACTIVE DEV-LEVEL VERBOSITY */
						...
						printf("\nmain-DEV-1456: check_tela via 2-D, princeps =%2d (+1 CONTINUITY, +2 EQUIVALENCE).", check_tela(0,p, 2));
						printf("\nmain-DEV-1457: check_tela via 1-D, princeps =%2d (+1 CONTINUITY, +2 EQUIVALENCE).", check_tela(0,q, 1));
	...
	if (options[1][57] > 3) {	/* opt_v DEV-LEVEL VERBOSITY */
		...
		if (n!=3)
			printf("\nmain-DEV-1526: check_tela via 1-D coords, princeps = %2d (<3).", n);
		n = check_tela(0,citwidth,2);
		if (n!=3)
			printf("\nmain-DEV-1529: check_tela via 2-D coords, princeps = %2d (<3).", n);
		print_tela(prtela_A, prtela_B);
	}
