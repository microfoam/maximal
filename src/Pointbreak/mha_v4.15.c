...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.15";				/* current version number */
	...
	for (n = 1; n<=lenseq; ) {
		...
		for (m = 0; m < n; m++) {
			...
			if (pathbox[m][n] > mismatch && n+k <= lenseq) {
				...
				if (tela[n].all_L && check_tela(tela[n].all_L,lenseq, ONE)!=3) {
					Dtr = imperfect_TR = 0;
				}
				else if (check_tela(m,lenseq, ONE)!=3) {
					Dtr = imperfect_TR = 0;
				}
				...
				if (Dtr) {
					if (imperfect_TR) {
						 o = push_tela(n,m, THREE);			/* WITH MODE ONE WILL ASSIGN TRANSITIONS WITHIN PUSH_TELA */
					}
					else {
...
*/					}
					...
					assign_tela(n, row, a2D_n, OFF);	/* MODES O-F-F, ONE=FLATLINE AT N, TWO=ASSIGN  */
					r = 0;
					if (badslip_type >= 100) {
						tela[bad_1Dn].r = 0;
					}
