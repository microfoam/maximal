...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.07";				/* current version number */
						...
						if (r<reps) {
							...
							if (!badslip_type) {
								...
								if (sumspan > 0 && tela[n].X != n) {	/* SUMSPAN IS LENGTH OF WINDOWS FOR WHICH SUMS OF PRODUCTS ARE RECORDED */
									...
									int cycpos = z;					/* CYCLING POSITION, FOR CLARITY. WILL NOT STAY AT z */
									for (j = 1; j < -sumspan; j++) {
										if (tela[z+j].cyc_S > c) {
											c = tela[(cycpos=z+j)].cyc_S;
										}
									}
									if (cycpos != z) {
										if (cyclelize_tela(z, cycpos-z, n)) {	/* REMINDER: cyclelize_tela(int cpos, int delta, int npos) */
											badslip_type = 50;					/* FROM SEQUENCE IN TYPES: 1-3-5-10-30- (50) -100-300-500 */
											options[1][50] += badslip_type;
											options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
											if (options[1][57] > 3) /* opt_v DEV-LEVEL VERBOSITY */
												printf("\nDEV-1169: badslip type %d at n=%d for k=%d with TR at cycpos=%d.", badslip_type, n, k, cycpos);
		...
		for (m = 0; align2D[m][0] != '\0'; m++) {
			...
			for (n = 0; align2D[m][n] != '\0'; n++) { 
				...
				if (check_imperf) { 
					...
					if (l == k && (i=100*sum4score/(k*MATCH)) > score_DTHR(k)) {
						imperfect_TR = 1;
					}
	...
	for (n = n_start; n <= n_end; n++) {
		...
		for (m = 0; align2D[m][0] != '\0'; m++) {
			if (isalpha(letr=align2D[m][n])) {
				...
				else if (isupper(letr) && letr != consensus_ar[1][n+1]) {
		...
		if (checktransit) {		/* IF checktransit IS STILL POSITIVE THEN IT'S SUPPOSED TO NOT HAVE CHECKED OUT */
			if (options[1][57]>2) {		/* opt_v VERBOSITY */
				printf("DEV-3392: check_transit=%d at n=%d.\n", checktransit, n);
