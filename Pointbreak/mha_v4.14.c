...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.14";				/* current version number */
		...
		for (m = 0; m < n; m++) {
			...
			if (pathbox[m][n] > mismatch && n+k <= lenseq) {
				...
				if (Dtr && dev_print(MAIN,__LINE__)) {
					printf("cinch_t for k=%d all_r=%d at n=%d, imperfect_TR=%d.", k, tela[n].all_r, n, imperfect_TR);
				}
							...
							for (l = n; l+k <= lenseq && l+k <= m+WIDTH; l++) {
								if (tela[l].c == tela[l+k].c) { 
									if (j <= l+1) {
										...
										tela[n].cyc_o = 'o';
										if (dev_print(MAIN,__LINE__)) {
											printf("Skipping cinch here to use later cycling frame at %d.", l+1);
										}
										break;
									}
								}
				...
				if (Dtr && (o=push_tela(n,m,3))) {	
					if (dev_print(MAIN,__LINE__)) {
						printf("push_tela violations=%d (+1 CONT, +2 EQUIV). Skipping k=%d-mer at n=%d.", o, k, n+1);
				...
				/* IF SUMMING PATHBOX DIAGONAL 8/:  1st MEASUREMENT OF TANDEM REPEAT (TR) */ 
							...
							if (!badslip_type) {
								...
								if (sumspan > 0 && tela[n].X != n) {	/* SUMSPAN IS LENGTH OF WINDOWS FOR WHICH SUMS OF PRODUCTS ARE RECORDED */
									...
									if (l != z && tela[(tela[l].cyc_Lf)].cyc_o == 'x') {
										...
										o = push_tela(l,l-k, 3);
										if (dev_print(MAIN,__LINE__)) {
											printf("push_tela violations = %d.", o);
										}
										...
										if (tela[i].cyc_o == 'x') {
											if (cyclelize_tela(i, j-i, n)) {
												...
												if (dev_print(MAIN,__LINE__)) {
													printf("badslip type 10000 at n=%d; failed cyclelize_tela().", n);
												}
											}
										}
									}
                            ...
                            if (tela[i].r) {
                                if (!badslip_type) {
                                    ...
                                    if (i>m && i - tela[i].k < m && i%tela[i].k != m%tela[i].k) {
										...
										options[1][39] = 1; sprintf(dev_notes, "bslip sum %d", (int) options[1][50]);
										if (dev_print(MAIN,__LINE__)) {
											printf("badslip type %d at n=%d for k=%d with TR at i=%d.", badslip_type, n, k, i);
										}
	        	                    ...
	        	                    /* THIS IS A BAD SLIP BECAUSE OF NON-COINCIDENT TRANSITIONS  */
										...
										/* THIS IS BAD SLIP TYPE 3 EXCEPT MISMATCH IS O-F-FSET BACK FROM tela[i].r */
										...
										/* THIS IS BAD SLIP WITH MISMATCH O-F-FSET FWRD FROM tela[i].r */
										else {
					...
					for (i = 0; i < r; i++) {
						...
						if (badslipspan && i==0) {			/* FLIP TO LOWERCASE IN FIRST UNIT */	
							if (islower(align2D[(p=row-1)][(q=a2D_n - k + overslip - 1)])) 		/* IF OVERLAPPING OLD BADSLIP */
								assign_tela(m-1, p, q, 0,0, TWO);		/* MODES O-F-F, ONE--FIVE: ONE=FLATLINE AT N, TWO=ASSIGN  */
																	/* WILL REWRITE AS UPPERCASE */
							for (j = 0; j < k; j++) { 
								o=m+j;
								p=row-1;
						...
						for (j = 0; j < k; j++) {
							o = n + k*i + j;
							q = a2D_n - k + overslip + j;
							assign_tela(o, row, q, 0,0, TWO);		/* MODES O-F-F, ONE--FIVE: ONE=FLATLINE AT N, TWO=ASSIGN  */
						}
					...
					if (dev_print(MAIN,__LINE__)) {
						...
						printf("\n check_tela via 2-D = %2d (+1 CONTINUITY, +2 EQUIVALENCE).\n", check_tela(0,p, TWO));
						printf("\n check_tela via 1-D = %2d (+1 CONTINUITY, +2 EQUIVALENCE).\n", check_tela(0,q, ONE));
						printf("\n print_tela for k=%d and r=%d at n=%d, overslip=%d.", k, r, n, overslip);
						print_tela(0,55);
						print_tela(55,110);
/*						print_tela(prtela_A, prtela_B);
*/					}
