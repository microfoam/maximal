...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.02";				/* current version number */
...
/*** FUNCTION 12 **********************************************************/
unsigned int foam_2D(char foam_align2D[][MAXROW], int n_start, int n_width)
{
int m=0, n=0, n_end; 
char letr;
...
short unsigned int lcl_opt_F;
	...
	for (j = 0; j < blocks2D; j++) {
		...
		if (options[0][41] && options[1][18] > 6 && mmsites == 0) {	
			foam_2D(align2D, j_start, cip_linewidth);
		}
