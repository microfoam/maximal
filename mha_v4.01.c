...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.01";				/* current version number */
	...
	for (k = WIDTH; k > 0; k--) {
		...
		for (m = 0; align2D[m][0] != '\0'; m++) {
			...
			for (n = 0; align2D[m][n] != '\0'; n++) { 
				...
				if (options[1][59]==5 && keep_checking && k==1) {		/* WORKS BUT BETTER TO HAVE MATCHING PART IN CINCH-D OR REMOVE TO FUNCTION */
				...
				if (check_imperf) { 
					...
					if (l == k && (i=100*sum4score/(k*MATCH)) > score_DTHR(k,options[1][59])) {
						imperfect_TR = 1;
					}
				...
				if (keep_checking || imperfect_TR) {
					...
					if (nuctransit) {
						if (col_isclear(align2D,n,m,1) < 0) { 
							...
							for (i = n-x+k; i+k < options[1][32]; i++) {
								align2D[MAXROW][i] = align2D[MAXROW][i+k];
							}
							align2D[MAXROW][i] = '\0';
						}
				...
				if (cinch_d_opt) {	/* cinch_d ENGINE **********************************************************************/
					if (first_write) {
						...
						if (letr == '>' && j-cid_ncol-1 < cidwidth) {
							...
							mha_writecons(align2D, cid_align2D);
						}
					} /* END OF IF first_write EQUALS ONE */
				} /*********************************************************************************************/
				else if (options[1][57] > 3) {		/* opt_v DEV-LEVEL VERBOSITY */
					if (imperfect_TR)
...
char blnk = options[1][11], letr=blnk, conletr=blnk;
int con_maxrows=26;
int consensus_ar[26][MAXROW] = {{0}};	 	/* COL n=0 FOR BIT FLAG */
	...
	for (n = n_start; n <= n_end; n++) {
		if (nuctransit) {
			...
			if ((conletr=align2D[MAXROW][n])=='R' || conletr=='Y') {
		...
		for (m = 1; align2D[m][0] != '\0'; m++) {
			if (isalpha(letr=align2D[m][n]) && isupper(letr)) {
	...
	/* STORE CONSENSUS ROW */
	for (n = n_start; n <= n_end; n++) {
		align2D[MAXROW][n] = consensus_ar[1][n+1];
	}

	magiclen = 1000;		/* MAGIC LENGTH NUMBER: 100 bp per bad site; SEARCH MAGIC TO FIND OTHER STOPGAPS */
	if (options[1][18] == 7 && badsites>0 && con_width>magiclen && con_width/badsites < magiclen) {
		warnhead('C');
...
int m=0, n=0, widest_n=0;
	...
	clear_right(lclalign2D); 
	clear_2D_ar(align2D);
	for (m = 0; lclalign2D[m][0] != '\0' && m < MAXROW; m++) {
		for (n = 0; (letr=lclalign2D[m][n]) != '/' && letr != '>' && letr != wopt_R_rght; n++) {
			align2D[m][n] = letr;
			if (letr == wopt_Q_left && lclalign2D[m][n+wb_mwrap+1] == wopt_R_rght ) 
				align2D[m][n+wb_mwrap+2] = '\0';
		}
...
short unsigned int lcl_opt_F;
	...
	for (j = 0; j < blocks2D; j++) {
		...
		if (options[1][20] > 1 && options[1][18] > 6 && mmsites == 0) {
			foam_2D(align2D, options, j_start, cip_linewidth);
		}
	} /* END OF FOR j PRINTING LOOP */

	if (c == options[1][1] && mmsites == 0) {
		options[0][10] = 1000;	
...
char rlx_align2D[MAXROW+1][MAXROW];
	...
	mha_writecons(align2D, rlx_align2D); 
	...
	mha_writecons(rlx_align2D, align2D); 
...
{
	printf("This is maximal version %s, a program for micro-homology alignment (MHA).\n", usage_version);
...
char letr;
int bad_m = options[1][48];	/* THESE ARE FILLED BY consensus() */
int bad_n = options[1][49];	/* THESE ARE FILLED BY consensus() */
char badletr = align2D[bad_m][bad_n];
char blank = options[1][11];
...
unsigned short int nuctype = options[1][13], nuctransit=0;
	...
	mha_writeback(align2D, lcl_align2D);
	mha_writecons(align2D, lcl_align2D);
	...
	if (tuckcall==-1) {
		...
		if (pushdown(lcl_align2D, bad_m, bad_n+1)) {
			mha_writeback(lcl_align2D, align2D);
			mha_writecons(lcl_align2D, align2D);
			for (j=0; j < j_stop; j++) {
				align2D[bad_m][j  ] = blank;
			}
				align2D[bad_m][j  ] = badletr;
				align2D[bad_m][j+1] = '/';
				align2D[bad_m][j+2] = '\0';
		...
		if (pushdown(lcl_align2D, bad_m+1, 0)) {
			mha_writeback(lcl_align2D, align2D);
			mha_writecons(lcl_align2D, align2D);
				align2D[bad_m  ][bad_n   ] = '/';
				align2D[bad_m  ][bad_n+1 ] = '\0';
				align2D[bad_m+1][j_stop+1] = '/';
				align2D[bad_m+1][j_stop+2] = '\0';
			return(1);			/* RETURN THAT TUCKSENSE SHOULD BE INVOKED AGAIN */
		}
		else
			return(0);
