...
int main(int argc, char *argv[])
{
	...
	int mismatch   = -1;
	char version[] = "4.06";				/* current version number */
	...
	mark_all();			/* WILL MARK ALL TRs WITHOUT CINCHING AND RECORD IN tela[].all_k and tela[].all_r */
	for (n = 1; n<=lenseq; ) {
					...
					if (tela[n].X != n) {
						for (l=n-1; l>=tela[n].X; l--) {
							if ((q=tela[l].k) && (l + (p=span_rk(l)) - q) > m) { 
								...
								else if (tela[n].all_S < tela[l].all_S || 	/* SKIP THIS SLIP IF OVERLAPPING IN CONFLICT PREV. TR W/ > SCORE OR */
...
void mark_all(void) 
{
	...
	int threshold=0;
	int lenseq = options[1][1];
	unsigned short int nuctype = options[1][13], nuctransit=0, TRcheck=0, imperfect_TR=0, Aimperfect_TR=0;
